...
module Make(Io : Types.Io) = struct
  ...
  let put_common ?credentials ?connect_timeout_ms ?(confirm_requester_pays=false) ?unsigned_payload ~endpoint ?content_type ?content_encoding ?acl ?cache_control ?expect ?precondition ?(meta_headers=[]) ~bucket ~key ~body () =
    let path = sprintf "/%s/%s" bucket key in
    let precondition_header = match precondition with
      | None -> []
      | Some `If_none_match -> ["If-None-Match", "*"]
      | Some (`If_match etag) -> ["If-Match", etag]
    in
    let headers =
      (
      ...
      |> List.rev_append precondition_header
  ...
  module Stream = struct
    ...
    let put ?credentials ?connect_timeout_ms ?confirm_requester_pays ~endpoint ?content_type ?content_encoding ?acl ?cache_control ?expect ?precondition ?meta_headers ~bucket ~key ~data ~chunk_size ~length () =
      let body = Body.Chunked { length; chunk_size; pipe=data } in
      put_common ~endpoint ?credentials ?connect_timeout_ms ?confirm_requester_pays ?content_type ?content_encoding ?acl ?cache_control ?expect ?precondition ?meta_headers ~bucket ~key ~body ()
  end
  (* End streaming module *)

  let put ?credentials ?connect_timeout_ms ?confirm_requester_pays ~endpoint ?unsigned_payload ?content_type ?content_encoding ?acl ?cache_control ?expect ?precondition ?meta_headers ~bucket ~key ~data () =
    let body = Body.String data in
    put_common ?credentials ?connect_timeout_ms ?confirm_requester_pays ?unsigned_payload ?content_type ?content_encoding ?acl ?cache_control ?expect ?precondition ?meta_headers ~endpoint ~bucket ~key ~body ()

  let put_bigstring ?credentials ?connect_timeout_ms ?confirm_requester_pays ~endpoint ?unsigned_payload ?content_type ?content_encoding ?acl ?cache_control ?expect ?precondition ?meta_headers ~bucket ~key ~data () =
